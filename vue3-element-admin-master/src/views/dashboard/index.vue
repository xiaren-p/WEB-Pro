...
import { LogAPI, type VisitStatsVO, type VisitTrendVO } from "@/backend";
import { useUserStore } from "@/store/modules/user-store";
import { formatGrowthRate } from "@/utils";
import { useTransition, useDateFormat } from "@vueuse/core";
import { Connection, Failed } from "@element-plus/icons-vue";
// 使用纯 WebSocket 在线用户统计（Channels）替换 STOMP 版本
import { usePlainOnlineCount } from "@/composables/websocket/usePlainOnlineCount";

// 在线用户数量组件相关（纯 WebSocket）
const { onlineUserCount, lastUpdateTime, isConnected, initWebSocket, closeWebSocket } =
  usePlainOnlineCount();
